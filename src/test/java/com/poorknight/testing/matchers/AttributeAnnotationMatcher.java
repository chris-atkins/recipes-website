...
/**
 * Responsible for checking whether a class has fields (attributes) that are annotated with a specific annotation. It is
 * also possible to pass other Matcher<Field> objects that will be applied to each field that has the annotation being
 * checked for.
 *
 * Ex: It would be possible to check whether the fields with the @AttributeBeingSearchedFor are of type String by
 * passing in a 'typeOf(String.class)' matcher. (once making such a matcher)
 */
public class AttributeAnnotationMatcher extends TypeSafeDiagnosingMatcher<Class<?>> {
	...
	@SafeVarargs
	private AttributeAnnotationMatcher(final Class<? extends Annotation> annotationClass, final Cardinality cardinality,
	...
	@SafeVarargs
	@Factory
	public static AttributeAnnotationMatcher hasAnnotationOnAttribute(final Class<? extends Annotation> annotationClass,
			final Cardinality cardinality, final Matcher<Field>... furtherMatchersToApplyToAnnotatedAttributes) {

		return new AttributeAnnotationMatcher(annotationClass, cardinality, furtherMatchersToApplyToAnnotatedAttributes);
	}
	...
	public void describeTo(final Description description) {
		...
		description.appendText("has ")//
				...
				.appendText(" attribute in the class with the @")//
	...
	protected boolean matchesSafely(final Class<?> classBeingInspected, final Description mismatchDescription) {
		...
		for (final Matcher<Field> matcher : this.furtherMatchersToApplyToAnnotatedAttributes) {
			if (doesNotMatchAppendingMismatchDescriptionsAppendingDescriptions(matcher, mismatchDescription)) {
				return false;
			}
		}
	...
	private boolean doesNotMatchAppendingMismatchDescriptionsAppendingDescriptions(final Matcher<Field> matcher, final Description mismatchDescription) {
