...
public class MethodTransactionAnnotationMatcher extends TypeSafeDiagnosingMatcher<Class<?>> {
	...
	static {
		TRANSACTIION_TYPE_TO_EXPECTED_ANNOTATION_TYPES = new HashMap<MethodTransactionAnnotationMatcher.TransactionType, List<TransactionAttributeType>>();
	...
	private boolean noPublicMethodWithGivenNameAndAnnotationExists(final Class<?> classToInspect) {
		return !MethodAnnotationMatcher.hasAnnotationOnMethod(TransactionAttribute.class, this.methodName).matches(classToInspect);
	}
